...
function launchMainAppWindow(isVisible) {
  ...
  mainWindow.webContents.on('dom-ready', () => {
    const externalJsPath = path.join(__dirname, 'mod.js'); // Ensure mod.js is in the same folder as mainScreen.js
    fs.readFile(externalJsPath, 'utf8', (err, data) => {
        if (err) {
            console.error('Failed to load mod.js:', err);
        } else {
            mainWindow.webContents.executeJavaScript(data)
                .then(() => console.log('External script executed successfully!'))
                .catch(console.error);
        }
    });
});
